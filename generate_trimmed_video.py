...
import os
import numpy as np
bayesian_frames_folder = 'bayesian_output'
...
writer = cv2.VideoWriter(os.path.join(trimmed_video_folder, 'trimmed_video.avi'), 0, 30, (width, height))
...
for i in range(len(b_frames_array)):
    ...
    if np.sum(image) >= threshold*height*width*layers*255:
      input_image = cv2.imread(os.path.join(i_frames_array, i_frames_array[i]))
