...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pretrained_3d", type=str, default="pretrained\c3d.pickle", help="load default 3D pretrained model."
    )
        ...
        for data, clip_idxs, dirs, vid_names in data_iter:
            ...
            for i, (_dir, vid_name, clip_idx) in enumerate( zip(dirs, vid_names, clip_idxs) ):
                ...
                features_writer.write(
                    feature=outputs[i], video_name=vid_name, idx=clip_idx, dir=args.save_dir,
                )
            f.add(str(vid_names[0]) + '.mp4 100 Category -1 -1 -1 -1 \n')
        
        annotation_file = open("Test_Annotation.txt", "w")
        for name in f:
            annotation_file.write(name)
